...
//		}
	...
	protected void importExampleProjectAndAssertNoErrorMarker(String projectType,
		...
		shell.activate();
		SWTBotTreeItem examplesNode = bot.tree().expandNode("Examples");
		waitForNodes(examplesNode);
		examplesNode.expandNode("Xtraitj Examples")
				.select(projectType);
		bot.button("Next >").click();
	...
	protected SWTBotTree outlineTree() {
		return outline().bot().tree();
	}
	
	protected SWTBotTreeItem outlineTraitNode(String name) {
		return outlineTree().getTreeItem(name);
	}
